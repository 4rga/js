...
/* ===== Pindah tombol ke bawah gambar pertama (saat tombol DIKLIK) ===== */
(function(){
  ...
  if(btn){
    btn.addEventListener('click', function(){
      // 1) Pindahkan tombol tepat di bawah gambar pertama
      var firstBlock=findFirstImageBlock();
      if(firstBlock) insertAfter(btn, firstBlock);

      // 2) Linkify semua ayat
      var n=linkifyAll(body);

      // 3) Ubah status tombol -> hijau & disabled
      try{
